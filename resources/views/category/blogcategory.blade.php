                        ...
                        <table class="table table-bordered text-center " id="example">
                          ...
                          <tbody class="text-center">
                                @foreach ($blogcategorys as $blogcategor)
                                  <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td>{{ $blogcategor->blogcategory_name }}</td>
                                    <td>{{ App\User::find($blogcategor->added_by)->name  }}</td>
                                    <td>{{ $blogcategor->created_at }}</td>
          ...
          <div class="col-md-4">
              <div class="card">
                  ...
                  <div class="card-body">
                      ...
                      @endif
                      <form method="post" action="{{  route('blog_category') }}">
                        @csrf
                          <div class="form-group">
                            <label for="exampleInputName">Blog Category Name</label>
                            <input type="text" class="form-control" placeholder="Enter Category Name" name="blogcategory_name">
                            {{-- @error ('product_name')
